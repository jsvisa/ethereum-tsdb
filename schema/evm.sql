...
CREATE TABLE IF NOT EXISTS {{ chain }}.blocks (
    ...
    sha3_uncles             CHAR(66) NOT NULL,  -- SHA3 of the uncles data in the block
    ...
    PRIMARY KEY (blknum, block_timestamp)
);
...
CREATE TABLE IF NOT EXISTS {{ chain }}.txs (
    ...
    PRIMARY KEY (txhash, block_timestamp)
);
...
CREATE TABLE IF NOT EXISTS {{ chain }}.traces (
    ...
    PRIMARY KEY (txhash, trace_address, block_timestamp)
);
...
CREATE TABLE IF NOT EXISTS {{ chain }}.logs (
    ...
    PRIMARY KEY (txhash, logpos, block_timestamp)
);
...
CREATE TABLE IF NOT EXISTS {{ chain }}.contracts (
    ...
    is_proxy                    BOOLEAN,    -- Whether this contract is EIP1947/EIP897 or other proxy
    upstream                    TEXT,       -- The upstream address of this token
    created_at                  TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at                  TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,

    PRIMARY KEY (address, txhash, block_timestamp)
);
...
CREATE TABLE IF NOT EXISTS {{ chain }}.erc1155_xfers (
    ...
    PRIMARY KEY (txhash, logpos, id_pos, block_timestamp)
);
